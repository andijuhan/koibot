...
const timer = async (client, groupId) => {
   if (config.count < 0) {
      ...
      if (config.count === 10) {
         client.sendMessage(
            groupId,
            '*[BOT]* Lelang *closed* dalam 9 menit 59 detik.'
         );
      }
      if (config.count === 9) {
         client.sendMessage(
            groupId,
            '*[BOT]* Lelang *closed* dalam 8 menit 59 detik.'
         );
      }
      if (config.count === 8) {
         client.sendMessage(
            groupId,
            '*[BOT]* Lelang *closed* dalam 7 menit 59 detik.'
         );
      }
      if (config.count === 7) {
         client.sendMessage(
            groupId,
            '*[BOT]* Lelang *closed* dalam 6 menit 59 detik.'
         );
      }
      if (config.count === 6) {
         client.sendMessage(
            groupId,
            '*[BOT]* Lelang *closed* dalam 5 menit 59 detik.'
         );
      }
      if (config.count === 5) {
         client.sendMessage(
            groupId,
            '*[BOT]* Lelang *closed* dalam 4 menit 59 detik.'
         );
      }
      if (config.count === 4) {
         client.sendMessage(
            groupId,
            '*[BOT]* Lelang *closed* dalam 3 menit 59 detik.'
         );
      }
      if (config.count === 3) {
         client.sendMessage(
            groupId,
            '*[BOT]* Lelang *closed* dalam 2 menit 59 detik.'
         );
      }
      if (config.count === 2) {
         client.sendMessage(
            groupId,
            '*[BOT]* Lelang *closed* dalam 1 menit 59 detik.'
         );
      }
      if (config.count === 1) {
         client.sendMessage(groupId, '*[BOT]* Lelang *closed* dalam 59 detik.');
      }
      setTimeout(() => {
         config.count - 1;
